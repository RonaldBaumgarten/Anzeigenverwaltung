        ...
        <div class="container-lg" style="max-width: 800px">
            ...
            @foreach($companies as $company)
                <div class="feature col-lg-6 themed-grid-col">
                    ...
                    <a href="{{ route('companies.show', $company) }}" class="btn btn-primary align-bottom">Details ansehen</a>
                    @if (Auth::user())
                    <a href="{{ route('companies.edit', $company) }}" class="btn btn-primary">Bearbeiten</a>
                    <form action="{{ route('companies.destroy', $company) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger align-bottom">Löschen</button>
                    @endif
                </div> 
            @endforeach
            </div>
